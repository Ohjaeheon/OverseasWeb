...
import { FileUp, Search, Trash2, Clock, ShieldCheck, Monitor, FileText, HardDrive, ChevronLeft, ChevronRight } from 'lucide-react';

export const AdminFileUploadLogsPage: React.FC = () => {
  ...
  const loadLogs = () => {
    adminService.getFileUploadLogs(searchTerm)
      .then(setLogs)
      .catch(console.error);
  };
  ...
  const handleClear = () => {
    if (!window.confirm("파일 업로드 로그 기록을 전체 삭제하시겠습니까?\n이 작업은 되돌릴 수 없습니다.")) return;
    adminService.clearFileUploadLogs()
      .then(() => {
        loadLogs();
        setCurrentPage(1);
      })
    ...
    <div>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px', flexWrap: 'wrap', gap: '16px' }}>
        <div>
          ...
          <p style={{ color: '#6b7a99', fontSize: '0.88rem', margin: '4px 0 0 0' }}>
            해외선교부 업무포털 및 주간예배 자동 취합 등을 통해 시스템에 업로드된 파일의 상세 로그를 조회합니다.
          </p>
        </div>
      ...
      <div style={{
        ...
        <div style={{ fontSize: '0.9rem', color: '#475569', fontWeight: 700 }}>
          총 <span style={{ color: '#2563eb' }}>{logs.length}</span>개의 업로드 기록이 조회되었습니다.
        </div>
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left', fontSize: '0.88rem' }}>
          <thead>
            <tr style={{ background: '#f8fafc', borderBottom: '2px solid #e2e8f0', color: '#475569' }}>
              ...
              <th style={{ padding: '14px 18px', fontWeight: 700 }}>
                <span style={{ display: 'inline-flex', alignItems: 'center', gap: '4px' }}>
                  <FileText size={15} color="#2563eb" /> 업로드 파일명
                </span>
              </th>
              <th style={{ padding: '14px 18px', fontWeight: 700 }}>
                <span style={{ display: 'inline-flex', alignItems: 'center', gap: '4px' }}>
                  <HardDrive size={15} color="#2563eb" /> 업로드 용량
                </span>
              </th>
              <th style={{ padding: '14px 18px', fontWeight: 700 }}>
                <span style={{ display: 'inline-flex', alignItems: 'center', gap: '4px' }}>
          ...
          <tbody>
            {paginatedLogs.length === 0 ? (
              <tr>
                <td colSpan={6} style={{ padding: '30px', textAlign: 'center', color: '#6b7a99' }}>
                  기록된 업로드 로그 데이터가 없습니다.
                </td>
              </tr>
            ) : (
