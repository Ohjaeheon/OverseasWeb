...
interface EditorGroup {
  groupLabel: string; // '' = 최상단(그룹 없음)
  items: SidebarItem[];
}

function flatToGroups(flat: SidebarItem[]): EditorGroup[] {
...
function groupsToFlat(groups: EditorGroup[]): SidebarItem[] {
...
export const AdminMenuLayoutPage: React.FC = () => {
  const { getMsg, reload } = useMessageDictionary();
  const [groups, setGroups] = useState<EditorGroup[]>(() => flatToGroups(menuLayoutService.getAdminMenuLayout()));
  ...
  const usedKeys = useMemo(() => {
    ...
    groups.forEach((g) => g.items.forEach((it) => { if (it.s) set.add(it.s); }));
    return set;
  }, [groups]);

  const catalogUnused = useMemo(
    () => ADMIN_MENU_CATALOG.filter((it) => it.s && !usedKeys.has(it.s)),
    [usedKeys]
  );
  ...
  const startEditGroup = (groupIdx: number) => {
    ...
    setEditValue(getMsg(menuKeyForAdminGroup(groupKeyOf(g.groupLabel)), g.groupLabel).replace(/^__/, ''));
    setEditing({ kind: 'group', groupIdx });
  };

  const startEditItem = (groupIdx: number, itemIdx: number) => {
    const it = groups[groupIdx].items[itemIdx];
    setEditValue(getMsg(menuKeyForAdminItem(it), it.label || '').replace(/^__/, ''));
  ...
  const handleSave = async () => {
    setSaving(true);
    try {
      await menuLayoutService.saveAdminMenuLayout(groupsToFlat(groups));
  ...
  const rowLabelInput = (value: string, onConfirm: () => void) => (
  ...
  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px', flexWrap: 'wrap', gap: '16px' }}>
        <div>
          <h1 style={{ fontSize: '1.5rem', fontWeight: 800, color: '#1f2a44', margin: 0 }}>🗂️ 메뉴 배치 관리</h1>
          <p style={{ color: '#6b7a99', fontSize: '0.88rem', margin: '4px 0 0 0' }}>
            관리자 포탈 사이드바의 그룹/메뉴 순서를 드래그로 바꾸고, 이름은 항목 옆 ✏️로 바로 수정할 수 있습니다.
            왼쪽 카탈로그의 페이지를 오른쪽 그룹으로 끌어다 놓으면 메뉴에 추가됩니다.
          </p>
        </div>
        <button
          onClick={handleSave}
      ...
      <div style={{ display: 'flex', gap: '20px', alignItems: 'flex-start', flexWrap: 'wrap' }}>
        ...
        <div style={{ ...cardStyle, width: '280px', flexShrink: 0, padding: '16px' }}>
          ...
          <div style={{ fontSize: '0.78rem', color: '#94a3b8', marginBottom: '10px' }}>
            이미 만들어진 관리자 페이지 중 현재 사이드바에 없는 항목입니다. 드래그해서 오른쪽 그룹에 놓으세요.
          </div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '6px', minHeight: '40px' }}>
            {catalogUnused.map((it) => (
              ...
              >
                ...
                <span>{it.ico}</span>
                <span>{getMsg(menuKeyForAdminItem(it), it.label || '').replace(/^__/, '')}</span>
              </div>
            ))}
                ...
                {editing?.kind === 'group' && editing.groupIdx === groupIdx ? (
                  ...
                  <>
                    <span style={{ fontWeight: 800, color: g.groupLabel ? '#1f2a44' : '#94a3b8', fontSize: '0.85rem', textTransform: g.groupLabel ? 'uppercase' : 'none' }}>
                      {g.groupLabel ? getMsg(menuKeyForAdminGroup(groupKeyOf(g.groupLabel)), g.groupLabel).replace(/^__/, '') : '최상단 (그룹 없음)'}
                    </span>
                    {g.groupLabel && (
                      <>
